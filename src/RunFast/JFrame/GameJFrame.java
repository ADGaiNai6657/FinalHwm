...
public class GameJFrame extends JFrame { //游戏主窗口，负责跑得快发牌、出牌和界面显示
    ...
    private final int[] scores = new int[3];                                   //三个玩家的积分，索引和玩家索引保持一致
    ...
    private final Random random = new Random();                                //随机对象，用于洗牌和随机生成特殊牌

    private int turn = 1;                 //0为左侧电脑，1为玩家，2为右侧电脑
    private int lastPlayer = -1;          //最后成功出牌的玩家
    private int previousWinner = -1;      //上一局胜者，第一局时为-1
    ...
    private void initView() {
        ...
        playerInfo[0].setBounds(20, 70, 170, 44);
        playerInfo[1].setBounds(355, 560, 220, 28);
        playerInfo[2].setBounds(740, 70, 170, 44);

        playButton.setBounds(350, 475, 90, 30);
        passButton.setBounds(490, 475, 90, 30);
        restartButton.setBounds(790, 500, 110, 32);
    ...
    private void startNewGame() {
        ...
        //第一局由梅花3先出，后续由上一局胜者先出
        turn = previousWinner >= 0 ? previousWinner : findClubThreePlayer();
        statusLabel.setText(playerName(turn) + " 先出牌");
        renderTable();
        updateControls();
        //如果先手是电脑，则自动执行电脑回合
        computerTurnLater();
    ...
    private void renderTable() {
        ...
        renderDiscardPiles();
        updatePlayerInfo();
        //牌组件会被放到顶层，所以最后再把操作按钮提到最上层
        bringPlayControlsToFront();

        container.repaint();
    }

    //此方法的目的是让出牌和不要按钮始终显示在扑克牌上方
    private void bringPlayControlsToFront() {
        container.setComponentZOrder(playButton, 0);
        container.setComponentZOrder(passButton, 0);
    }
    ...
    //此方法的目的是刷新三个玩家的剩余牌数和积分
    private void updatePlayerInfo() {
        //电脑玩家面前空间较窄，牌数和积分分两行显示
        playerInfo[0].setText("<html><div style='text-align:center;'>电脑A：" + playerList.get(0).size() + "张<br>积分：" + scores[0] + "</div></html>");
        playerInfo[1].setText("玩家：" + playerList.get(1).size() + "张  积分：" + scores[1]);
        playerInfo[2].setText("<html><div style='text-align:center;'>电脑B：" + playerList.get(2).size() + "张<br>积分：" + scores[2] + "</div></html>");
    }
    ...
    private void checkWinner(int playerIndex) {
        ...
        if (choice == 0) {
            //延迟重开，避免旧回合调用栈继续推进新局先手
            SwingUtilities.invokeLater(this::startNewGame);
        }
    }

    //此方法的目的是在一局结束后更新三个玩家的积分
    private void updateScores(int winnerIndex) {
    ...
    //此方法的目的是第一局寻找持有梅花3的玩家
    private int findClubThreePlayer() {
        for (int i = 0; i < playerList.size(); i++) {
            for (Card card : playerList.get(i)) {
                if ("MeiHua".equals(card.getm_Color()) && "3".equals(card.getm_Number())) {
                    return i;
                }
            }
        }
